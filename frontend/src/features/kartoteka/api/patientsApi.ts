...
function matchesFilters(patient: Patient, filters?: PatientFilters): boolean {
  ...
  if (status !== 'svi' && patient.cardStatus !== status) return false
  ...
  if (filters.species && filters.species !== 'sve' && patient.species !== filters.species) {
    return false
  }
  if (filters.sex && filters.sex !== 'svi' && patient.sex !== filters.sex) {
    return false
  }
  if (filters.allergies && filters.allergies !== 'sve' && patient.allergies !== filters.allergies) {
    return false
  }
  if (filters.city && filters.city !== 'svi' && patient.city !== filters.city) {
    return false
  }
...
export async function softDeletePatient(id: string): Promise<void> {
  ...
  patients[index] = { ...patients[index], cardStatus: 'obrisan' }
