...
function HourRow({ entry, max, isPeak }: { entry: HourBreakdown; max: number; isPeak: boolean }) {
  ...
  return (
    <div className={styles.barRow}>
      ...
      <span className={styles.barCount}>{entry.count > 0 ? `${entry.count} term.` : '—'}</span>
    </div>
  )
}
...
export function PeakHoursPanel({ open, onOpenChange }: PeakHoursPanelProps) {
  ...
  return (
    <SlidePanel
      ...
      ariaLabel="Najtraženiji termini"
      header={
        <div>
          <div className={styles.title}>Najtraženiji termini</div>
          <div className={styles.subtitle}>Raspored zakazivanja po satu i po danu nedelje.</div>
        </div>
      }
    >
      {!data ? (
        ...
        <>
          <section className={styles.section}>
            <h3 className={styles.sectionTitle}>Pregled</h3>
            <div className={styles.overviewGrid}>
              <Tile label="Najtraženiji sat" value={data.peakHour?.hour ?? '—'} />
              <Tile label="Termina u tom satu" value={String(data.peakHour?.count ?? 0)} />
              <Tile label="Najprometniji dan" value={data.busiestDay?.day ?? '—'} />
              <Tile label="Prosečno po danu" value={data.averagePerDay.toFixed(1)} />
            </div>
          </section>

          <section className={styles.section}>
            <h3 className={styles.sectionTitle}>Termini po satu</h3>
          ...
          <section className={styles.section}>
            <h3 className={styles.sectionTitle}>Po danu nedelje — ukupno i najtraženiji sat</h3>
            ...
            <p className={styles.note}>Računato na osnovu {data.totalAppointments} termina.</p>
          </section>
        </>
      )}
    </SlidePanel>
